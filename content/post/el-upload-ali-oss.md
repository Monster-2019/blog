...
阿里云 oss 如今是很常见的图床了，那么图片上传到阿里云 oss 保存也是很常见的需求，今天给大家分享 el-upload 组件上传到阿里云 oss 的封装，开箱即用。
...
<template>
  ...
  >
    <el-icon v-if="type === 'image' || limit < fileList.length" class="custom-uploader-icon"
      ><Plus
    /></el-icon>
...
const props = defineProps({
  ...
  originName: { // 上传文件名是否使用原名，默认用uuid避免重名
    type: Boolean,
    default: () => false,
  },
  drag: { // 是否可拖拽修改顺序
...
const handleUpload = async (config: any) => {
  try {
    ...
    const path = `/${curDate}/${
      props.originName ? fileName : v4().replace(/-/g, '')
    }.${ext}`;
...
file 参数就是我们自定义上传成功返回的数据。因为上传多个文件的时候，每次都会触发 on-success 事件，所以我们使用 uploadPool 临时存储已上传的 url，只有在一次性选择的所有文件都上传完成，我们才会更新数据。

> 注意：更新数据后我们需要清空 Upload 实例的上传列表，否则已上传文件会一直存在，导致超过最大上传数量
...
const handleSuccess = (file: any, fileList1: any) => {
  let { url } = file;
  uploadPool.value.push(url);
  if (upload.value.uploadFiles.find((file: any) => file.status !== 'success')) {
    return;
