...
description: 记录 Firebase Cloud Messaging 从旧版接口迁移到 HTTP v1 API 的关键步骤、鉴权方式，以及迁移过程中常见的报错与处理方式。
---
...
const httpsAgent = process.env.NODE_ENV
  ? new HttpsProxyAgent("http://xxx.xxx.xxx.xxx:xxxx")
  : null;
...
const sendNotification = (data, config) =>
  axiosInstance.post(
    "https://fcm.googleapis.com/v1/projects/xxxxxxx/messages:send",
    data,
    config
  );
